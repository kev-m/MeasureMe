...
def fetch_and_store_historical_data(db_url: str, start: date, end: date, only_types: list = None, tz_name: str = "Europe/London"):
    ...
    all_types = ['intraday_heart_rate', 'sleep', 'resting_heart_rate', 'breathing_rate', 'hrv', 'exercises', 'weight']
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Ingest historical data directly from Fitbit REST API into MeasureMe.")
    parser.add_argument('--db', type=str, default='sqlite:///storage/measureme_fb.db', help='SQLAlchemy Database URL')
    parser.add_argument('--start', type=str, required=True, help='Start date YYYY-MM-DD')
    parser.add_argument('--end', type=str, required=True, help='End date YYYY-MM-DD')
    parser.add_argument('--types', nargs='+', help='Specify which collections to import (sleep activities). Defaults to all supported.')
    parser.add_argument('--timezone', type=str, default="Europe/London", help='The IANA timezone to assume for the imported local times (e.g., Europe/London).')
    ...
    fetch_and_store_historical_data(args.db, s_date, e_date, args.types, args.timezone)