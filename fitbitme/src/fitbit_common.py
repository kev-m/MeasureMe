...
class FitbitDataMapper:
        ...
        for entry in sleep_entries:
            ...
            if not existing:
                ...
                potentials = self.db.query(models.SleepSession).filter(
                    ...
                    models.SleepSession.is_main_sleep == entry.get('isMainSleep', True),
            ...
            if not existing:
                session = models.SleepSession(
                    ...
                    deep_sleep_seconds=levels_summary.get('deep', {}).get('minutes', 0) * 60,
                    light_sleep_seconds=levels_summary.get('light', {}).get('minutes', 0) * 60,
                    rem_sleep_seconds=levels_summary.get('rem', {}).get('minutes', 0) * 60,
                    awake_seconds=levels_summary.get('wake', {}).get('minutes', 0) * 60,
                ...
                existing.deep_sleep_seconds = levels_summary.get('deep', {}).get('minutes', 0) * 60
                existing.light_sleep_seconds = levels_summary.get('light', {}).get('minutes', 0) * 60
                existing.rem_sleep_seconds = levels_summary.get('rem', {}).get('minutes', 0) * 60
                existing.awake_seconds = levels_summary.get('wake', {}).get('minutes', 0) * 60
    ...
    def process_activities(self, activities_data: dict, date_str: str):
        ...
        if summary:
            if 'steps' in summary:
                self.upsert_metric('steps', float(summary['steps']), 'count', dt)
            if 'restingHeartRate' in summary:
                self.upsert_metric('resting_heart_rate', float(summary['restingHeartRate']), 'bpm', dt)
    ...
    def process_heart_rate(self, hr_data: dict, date_str: str):
        ...
        for hr_day in hr_data.get('activities-heart', []):
            ...
            if rhr:
                self.upsert_metric('resting_heart_rate', float(rhr), 'bpm', dt_base)
        ...
        for point in hr_data.get('activities-heart-intraday', {}).get('dataset', []):
            ...
            if time_str and val is not None:
                ...
                dt_point = datetime.strptime(f"{date_str} {time_str}", "%Y-%m-%d %H:%M:%S").replace(tzinfo=self.tz_info)
    ...
    def process_breathing_rate(self, data: dict):
        for entry in data.get('br', []):
            ...
            if dt_str and val is not None:
                ...
                self.upsert_metric('breathing_rate', float(val), 'breaths/min', dt)
        self.db.commit()
        ...
        for entry in data.get('weight', []):
            ...
            if dt_str:
                ...
                if entry.get('weight') is not None:
                    self.upsert_metric('weight', float(entry.get('weight')), 'kg', dt)
                if entry.get('bmi') is not None:
                    self.upsert_metric('bmi', float(entry.get('bmi')), 'index', dt)
                if entry.get('fat') is not None:
                    self.upsert_metric('body_fat', float(entry.get('fat')), 'percent', dt)
        self.db.commit()
    ...
    def fetch_and_process(self, collection_type: str, date_str: str, is_webhook: bool = False):
        ...
        try:
            if collection_type == 'sleep':
                # Use Version 1.2 API 
                data = self.client.fetch_data(f"https://api.fitbit.com/1.2/user/-/sleep/date/{date_str}.json")
                log.debug(f"Raw sleep data from API: {json.dumps(data)}")
                log.info(f"Successfully fetched {len(data.get('sleep', []))} sleep records.")
                self.mapper.process_sleep(data)
                
                if is_webhook:
                    # Webhooks only tell us about 'sleep', not BR or HRV. We opportunistically grab them.
                    try:
                        br_data = self.client.fetch_data(f"br/date/{date_str}/all.json")
                        self.mapper.process_breathing_rate(br_data)
                    except Exception as e: log.debug(f"No breathing rate for {date_str}: {e}")
                    
                    try:
                        hrv_data = self.client.fetch_data(f"hrv/date/{date_str}.json")
                        self.mapper.process_hrv(hrv_data)
                    except Exception as e: log.debug(f"No HRV for {date_str}: {e}")
                ...
                data = self.client.fetch_data(f"activities/date/{date_str}.json")
                ...
                if collection_type == 'activities' and is_webhook:
                    ...
                    try:
                        hr_data = self.client.fetch_data(f"activities/heart/date/{date_str}/1d/1min.json")
                        self.mapper.process_heart_rate(hr_data, date_str)
                    except Exception as e: log.debug(f"No intraday HR for {date_str}: {e}")
                ...
                try:
                    body_data = self.client.fetch_data(f"body/log/weight/date/{date_str}.json")
                    self.mapper.process_weight(body_data)
                    log.info(f"Successfully fetched weight records for {date_str}.")
                except Exception as e: log.debug(f"No weight for {date_str}: {e}")

            else:
                log.warning(f"No specific handler yet for live sync of collection: {collection_type}")

        except Exception as api_err:
            log.error(f"API Error fetching {collection_type}: {api_err}", exc_info=True)
            raise
