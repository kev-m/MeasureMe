...
class FitbitClient:
    def __init__(self, client_id=None, client_secret=None):
        ...
        self.redirect_url = os.environ.get('FB_REDIRECT_URL', 'http://localhost:8080/callback')
        self.auth_url_base = os.environ.get('FB_AUTHORISE_URL', 'https://www.fitbit.com/oauth2/authorize')
        self.token_url = os.environ.get('FB_TOKEN_URL', 'https://api.fitbit.com/oauth2/token')
        self.tokens = {}
    ...
    def fetch_data(self, endpoint, is_retry=False):
        ...
        if not self.tokens.get('access_token'):
            raise Exception("No access token available. Please run initial auth hook.")
        ...
        url = endpoint if endpoint.startswith('http') else f"{FITBIT_API_BASE}/{endpoint}"
    ...
    def perform_initial_auth(self):
        ...
        if not self.client_id or not self.client_secret:
            print("Error: FB_CLIENT_ID and FB_CLIENT_SECRET map must be set in .env file.")
            sys.exit(1)
        ...
        print(f"3. You will be redirected to a {self.redirect_url} that will fail to load.")
        ...
        if resp.status_code == 200:
            ...
            print("Success! Tokens saved to tokens.json. The worker can now act autonomously.")
        else:
    ...
    def create_subscription(self, subscription_id="1"):
        ...
        print(f"Creating Fitbit user subscription with ID {subscription_id}...")
        ...
        if response.status_code in [200, 201]:
            ...
            print(f"Subscription '{subscription_id}' already exists for this user. You are ready to go!")
        else:
